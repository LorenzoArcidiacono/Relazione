...
\nl
Le modifiche impostate dal back office vengono comunicate al dispositivo che, tramite le interfacce offerte dal sistema, scrive i file necessari alla persistenza delle informazioni \torevise{spiegarsi meglio}, scarica i contenuti multimediali necessari e configura le impostazioni selezionate.}


La figura \ref*{fig:architettura} mostra in maniera schematica l'architettura dell'applicazione e l'interazione con il consumatore. \torevise{figura un po' confusa: che cosa è "back office" che è rappresentato con una nuvoletta? Inoltre, davvero c'è una copia del DB in ogni device? E poi, come mai è nel device? la freccia etichettata QR Code dovrebbe essere unidirezionale, o sbaglio? manca un modello dinamico, che aiuterebbe a documentare meglio. Tipo un diagramma di sequenza }
...
\torevise{scrivere meglio i requisiti. passo passo, uno alla volta. Con applicazione client intanto cosa si intende? chiarirlo che tipocamente il lato client sarebbe lato cellulare dell'utilizzatore. Il web engine dove risiede? Gli script dove sono eseguiti? E comunque questi non sono requisiti funzionali, ma vincoli sull'architettura. }

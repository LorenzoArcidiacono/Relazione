...
Nella Fig.\ref*{fig:architettura_2} è presentato uno schema dell'architettura dell'applicazione lato client \torevise{e lato server} in cui sono mostrate le componenti principali: Livesignage, il sistema operativo e il web server che ospita il back office. \torevise{la nuvoletta file system è ambigua, file system di quale macchina? oppure è nel cloud? dirlo. In caso perché il file system su cloud e i DB in locale?}
...
\begin{itemize}
    ...
    \item Livesignage: dopo aver stabilito il canale di comunicazione con il web server, resta in attesa dei messaggi da questo inviati e si occupa di svolgere le operazioni richieste \torevise{da chi?} comunicando, tramite le interfacce \torevise{usando le primitive definite ....}, con il sistema operativo sottostante.
    \item Sistema operativo: gestisce le richieste fatte da Livesignage tramite API, una volta che queste sono portate a termine chiama le funzioni di callback passate come parametri dell'API (vedi \ref*{apicode}). \torevise{i callback }
\end{itemize}
...
Il resto dei file fondamentali si trova nella cartella \jscode{js/}, qui sono raccolti gli script e le classi necessarie al corretto svolgimento dell'applicazione \torevise{non si svolge un'applicazione. La si impementa. oppure corretto funzionamento}:
...
Durante lo studio della documentazione LG è stato osservato che il buffer di memoria per la lettura e scrittura di un file è solo di 10 KB; è stato quindi necessario implementare le funzioni di scrittura e lettura di un file in maniera ricorsiva \torevise{spiegare perché ricorsiva e non iterativa}.
...
\torevise{in fase di presentazione giustificare meglio}
...
Un'altra importante funzionalità aggiunta è stata quella di poter cambiare la gestione dei segnali ricevuti dal telecomando \torevise{sbaglio, o è la prima volta che si parla di telecomando? parlarne anche nei requisiti} all'interno dell'applicazione.
...
Tutti i test sono stati svolti su entrambi i dispositivi: il media player si è dimostrato migliore per quanto riguarda la scrittura sul database mentre il display nei test del browser \torevise{frase che non sta in piedi in italiano}.
